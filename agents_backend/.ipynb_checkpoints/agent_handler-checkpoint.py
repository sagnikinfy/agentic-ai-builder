...
def crew_nodes(state, agent, name):
...
def human_review_node(state) -> Command[Literal["verify", "{agent_tag}"]]:
    ...
    if review_action == "continue":
        ...
        return Command(goto="{agent_tag}", update={{"messages" : [
            HumanMessage(content=review_data)
        ], "agent_history": []}})

    elif review_action == "update":
        return Command(goto="verify", update={{"messages" : [
            HumanMessage(content=review_data)
...
verify_node = functools.partial(crew_nodes, agent=verify_agent, name = "verify")
    """
    
    code += f"\n{agent_tag}_node = functools.partial(crew_nodes, agent={agent_tag}, name = '{agent_tag}')\n"
    code += f"""
def create_graph(memory):
    builder = StateGraph(State)
    builder.add_node("{agent_tag}",{agent_tag}_node)
    builder.add_node("verify",verify_node)
    builder.add_node("human_review_node",human_review_node)
    builder.add_edge(START, "{agent_tag}")
    builder.add_conditional_edges("{agent_tag}", route_after_llm)
    builder.add_conditional_edges("verify", route_after_llm)
...
def create_or_update_agent(agent_tag: str, skill_tags: List[str], system_prompt: str, mode: str) -> str:
    """
    ...
    if check_if_exists(agent_tag, "agents") and mode == "insert":
        ...
        agentsets[agent_tag] = {"prompt" : system_prompt, "tools": skill_tags}
